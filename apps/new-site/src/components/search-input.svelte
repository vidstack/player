<script lang="ts">
  ...
  onMount(() => {
    return listenEvent(document, 'keydown', (event) => {
      if ((event.metaKey && event.key === shortcutKey) || (shortcutSlash && event.key === '/')) {
...
<div class="relative flex-auto group">
  ...
  {#if shortcutKey || shortcutSlash}
    <div
      class="px-2 py-px font-mono text-sm font-bold border-border bg-elevate text-inverse flex items-center justify-center absolute right-1 top-2 rounded-sm"
    >
