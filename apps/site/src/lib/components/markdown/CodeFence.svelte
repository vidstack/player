...
>
  {#if showTopBar}
    <div
      class="supports-backdrop-blur:bg-white/60 sticky top-0 left-0 z-10 flex items-center rounded-md pt-2 backdrop-blur"
    ...
    >
      ...
      {#if showCopyCode}
        <button
          type="button"
          class="text-gray-soft mr-2 px-2 py-1 hover:text-white"
          on:click={copyCodeToClipboard}
        >
          <div
            class={clsx(
              'bg-brand-300/80 absolute top-2.5 right-4 z-10 rounded-md px-2 py-1 font-mono text-sm text-white transition-opacity duration-300 ease-out',
    ...
    {#if showLineNumbers}
      <pre
        ...
        style="background-color: transparent; border-radius: 0; padding-top: 0;">
			<div
          class="992:block hidden flex-none select-none text-right text-slate-600"
    ...
    >
      {#each lines as lineNumber}
        {#if isHighlightLine(lineNumber)}
          <div
            class="border-l-brand-300 bg-brand-200/5 w-full border-l-[5px] font-mono text-transparent"
          >
            &nbsp;
          </div>
        {:else}
