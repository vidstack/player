...
export type ExtractGlobalEventType<EventType> = EventType extends keyof GlobalEventHandlersEventMap
  ? GlobalEventHandlersEventMap[EventType]
  : Event;

export type ReactCallbacks<S> = {
  [P in keyof S]?: (e: ExtractGlobalEventType<S[P]>) => unknown;
};
...
export const createComponent = <I extends HTMLElement, E extends Record<string, string>>(
  ...
  type UserProps = React.PropsWithChildren<
    React.PropsWithRef<
      Partial<Omit<I, 'children'>> & ReactCallbacks<E> & React.HTMLAttributes<HTMLElement>
    >
  >;
